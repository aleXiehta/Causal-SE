from cmgan.conformer import ConformerBlock
from utils import *
...
class TSCNet(nn.Module):
    ...
    def forward(self, x):
        x = x.squeeze(1)
        spec = torch.stft(x, n_fft=self.n_fft, hop_length=self.hop_length, win_length=self.n_fft, window=torch.hann_window(self.n_fft).to(x.device).pow(0.5))
        spec = spec.transpose(1, 2)
        mag = spec.pow(2).sum(-1).pow(0.5).unsqueeze(1)
        # mag = torch.sqrt(x[:, 0, :, :]**2 + x[:, 1, :, :]**2).unsqueeze(1)
        spec = spec.permute(0, 3, 1, 2)
        noisy_phase = torch.angle(torch.complex(spec[:, 0, :, :], spec[:, 1, :, :])).unsqueeze(1)

        # mag = torch.sqrt(x[:, 0, :, :]**2 + x[:, 1, :, :]**2).unsqueeze(1)
        # noisy_phase = torch.angle(torch.complex(x[:, 0, :, :], x[:, 1, :, :])).unsqueeze(1)
        x_in = torch.cat([mag, spec], dim=1)
        ...
        final_real = final_real.permute(0, 3, 2, 1)
        final_imag = final_imag.permute(0, 3, 2, 1)
        final_spec = torch.cat([final_real, final_imag], dim=-1)
        final_wave = torch.istft(final_spec, n_fft=self.n_fft, hop_length=self.hop_length, win_length=self.n_fft, window=torch.hann_window(self.n_fft).to(x.device).pow(0.5), length=x.size(-1))
        # return final_real, final_imag
        return final_wave.unsqueeze(1)
