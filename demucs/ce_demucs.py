...
class CEDemucs(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder_t0 = nn.ModuleList()
        self.encoder_t1 = nn.ModuleList()
        self.decoder = nn.ModuleList()
        activation = nn.GLU(1) if glu else nn.ReLU()
        ch_scale = 2 if glu else 1
        ...
        for index in range(depth):
            ...
            self.encoder_t0.append(nn.Sequential(*encode))
            encode = []
            encode += [
                nn.Conv1d(chin, hidden, kernel_size, stride),
                nn.ReLU(),
            ...
            self.encoder_t1.append(nn.Sequential(*encode))
    ...
    def forward(self, mix, t):
        ...
        skips_t0 = []
        for i, encode in enumerate(self.encoder_t0):
            if i == 0:
                x0 = encode(x)
            else:
                x0 = encode(x0)
        ...
        skips_t1 = []
        for i, encode in enumerate(self.encoder_t1):
            if i == 0:
                x1 = encode(x)
            else:
                x1 = encode(x1)
        ...
        # trim/pad length
        if t.size(-1) < x0.size(-1):
            t = F.pad(t, (0, x0.size(-1) - t.size(-1)), mode='replicate')
        elif t.size(-1) > x0.size(-1):
            t = t[..., :x0.size(-1)]
        x = (1 - t) * x0 + t * x1

        for decode in self.decoder:
            skip_t0 = skips_t0.pop(-1)
            skip_t1 = skips_t1.pop(-1)

            # expand treatment
            margin = skip_t0.size(-1) % t.size(-1)
            num_repeat = skip_t0.size(-1) // t.size(-1)
            t_expand = t.repeat_interleave(num_repeat, dim=-1)
            if t_expand.size(-1) < x.size(-1):
                t_expand = F.pad(t_expand, (0, margin), mode='replicate')
            elif t_expand.size(-1) > x.size(-1):
                t_expand = t_expand[..., :x.size(-1)]

            x = x + (1 - t_expand) * skip_t0[..., :x.shape[-1]] + t_expand * skip_t1[..., :x.shape[-1]]
            x = decode(x)
        if self.resample == 2:
            x = downsample2(x)
        elif self.resample == 4:
